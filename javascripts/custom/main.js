(function () {
    ...
    $(function ($) {
        ...
        $(window).load(function () {
            ...
            } else { $('.animated').css('visibility', 'visible') }
        }); (function ($) { $.fn.filterPanelTrigger = function () { if ($('.works-filter-panel').is(':hidden')) { $('.works-filter-panel').slideDown() } else { $('.works-filter-panel').slideUp() } } })(jQuery)
        ...
        $(window).load(function () {
            $container1.isotope({ itemSelector: '.works-item', layoutMode: 'masonry', transitionDuration: '0.8s' })
        ...
        var setREVStartSize = function () {
            try {
                ...
                if (e.responsiveLevels && (jQuery.each(e.responsiveLevels, function (e, f) { f > i && (t = r = f, l = e), i > f && f > r && (r = f, n = e) }), t > r && (l = n)), f = e.gridheight[l] || e.gridheight[0] || e.gridheight, s = e.gridwidth[l] || e.gridwidth[0] || e.gridwidth, h = i / s, h = h > 1 ? 1 : h, f = Math.round(h * f), e.sliderLayout === 'fullscreen') {
            ...
            function map_init() {
                $('.contact').waypoint(function (direction) {
                    ...
                    if (direction === 'down' && !map_shown) {
                        ...
                        var marker = new google.maps.Marker({ position: myLatLng, map: map, title: 'Solo Dilo' })
                    } else { }
                }, { offset: 100 })
            }
        })
        ...
        var itemDiv = ''
        var isDrag = !1
        var cli = 0; function hoverOut($dato, $lista) {
        ...
        if ($(window).width() > 600) {
            $('.team-carousel-item').click(function (evt) {
                if (memoDiv.length > 0) {
                    ...
                    switch (imgClass) {
                        case 'team-01-carousel-item-01-hover': hoverOut(viewActive, memoDiv)
                            break
                    ...
                    var viewActive = $('.owl-item.active' || '.owl-item.cloned.active')
                    switch (imgClass) {
                        case 'team-01-carousel-item-01': idDiv.push(evt.currentTarget.id)
            ...
            $('.team-carousel-item').click(function (evt) {
                cli++; if (cli == 1) {
                    ...
                    var divEvt = evt.currentTarget.classList[1]
                    switch (divEvt) {
                        case 'team-01-carousel-item-01-hover': $(evt.currentTarget).removeClass()
        ...
        $(function ($) {
            ...
            $('.skills').waypoint(function (direction) {
                $('.progress-bar').each(function () {
                    ...
                    $(this).animate({ width: progressValue + '%' }, 2500)
                })
            }, { offset: '35%' }); (function ($) {
            ...
            $('.elements-counter-wrap').waypoint(function (direction) {
                if (direction === 'down') {
                    ...
                    $('.elements-counter').each(function () {
                        ...
                        $(this).find('.number').countTo({ from: 0, to: $endNum, speed: 2500, refreshInterval: 40 })
                    })
                } else { }
            }, { offset: '35%' })
        })
